...
async def chat_node(state: AgentState, config: RunnableConfig) -> Command[Literal["tool_node", "__end__"]]:
    ...
    # 1. Define the model (use centralized model factory)
    model = get_model()
    ...
    system_message = SystemMessage(content=persona)
    logger.info("Chat system message: %s", system_message.content)
    # 4. Run the model to generate a response (with error handling)
    messages = []
    try:
        recent = list(state["messages"])  # type: ignore[index]
        messages = [system_message, *recent[-8:]]
    except Exception:
